...
function ownerPlugin(owner) {
  return owner.split(":").slice(2).join(":");
}
...
function assertPluginOwnership(existing, plugin, destination, hash) {
  if (existing?.plugin && existing.plugin !== plugin && existing.hash !== hash) {
...
async function writeOwnedEntry({ source, destination, owner, plugin, kind, name, state, force, update }) {
  ...
  assertPluginOwnership(existing, plugin, destination, sourceHash);
...
async function writeGeneratedEntry({ content, destination, owner, plugin, kind, name, state, force, update }) {
  ...
  const existing = state.entries[destination];
  assertPluginOwnership(existing, plugin, destination, hash);
...
export async function uninstall(options) {
  ...
  for (const [path, entry] of Object.entries(state.entries)) {
    ...
    const entryPlugins = new Set([entry.plugin, ...(entry.owners ?? []).map(ownerPlugin)]);
    if (selectedPlugins && !selectedPlugins.has("all") && ![...selectedPlugins].some((plugin) => entryPlugins.has(plugin))) continue;
    ...
    const plugins = selectedPlugins && !selectedPlugins.has("all") ? [...selectedPlugins] : [...entryPlugins];
    const ownersToRemove = new Set(targets.flatMap((target) => plugins.map((plugin) => installationKey(options.scope, target, plugin))));
